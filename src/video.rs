...
use gtk4::prelude::*;
use gtk4::{Box as GtkBox, Button, Label, Orientation, Image, MenuButton, Popover, Scale, GestureClick, EventControllerKey};
use glib::clone;
use std::path::Path;
use std::rc::Rc;
...
impl GlobalVideoManager {
    ...
    fn register_player(&self, pipeline: &gstreamer::Pipeline, player_id: String) {
        ...
        if cleaned_count > 0 {
            println!("Video: Cleaned {} dead players during registration", cleaned_count);
        }
        
        // Add the new pipeline with its unique ID
        players.push((pipeline.clone(), player_id.clone()));
        
        println!("Video: Registered new player. Total active players: {}", players.len());
        println!("Video: New pipeline name: {} (ID: {})", 
    ...
    fn stop_other_players(&self, current_pipeline: &gstreamer::Pipeline, current_player_id: &str) {
        ...
        println!("Video: Checking {} registered players for stopping", players.len());
        println!("Video: Current pipeline name: {} (ID: {})", current_name, current_player_id);
        ...
        players.retain(|(pipeline, player_id)| {
            ...
            println!("Video: Checking pipeline '{}' (ID: {}) with state {:?}", pipeline_name, player_id, pipeline_state);
            
            // Remove if pipeline is NULL (destroyed)
            if pipeline_state == gstreamer::State::Null {
                println!("Video: Removing NULL pipeline: {} (ID: {})", pipeline_name, player_id);
                return false;
            }
            ...
            if player_id != current_player_id {
                ...
                if pipeline_state == gstreamer::State::Playing {
                    println!("Video: Stopping other playing video player: {} (ID: {})", pipeline_name, player_id);
                    ...
                    println!("Video: Pipeline '{}' (ID: {}) is not playing (state: {:?}), leaving as-is", pipeline_name, player_id, pipeline_state);
                }
            } else {
                println!("Video: Pipeline '{}' (ID: {}) is the current one, keeping it", pipeline_name, player_id);
            }
    ...
    fn stop_players_for_file(&self, file_path: &std::path::Path) {
        ...
        println!("Video: Stopping all players for file: {}", file_path.display());
        ...
        players.retain(|(pipeline, player_id)| {
            ...
            if player_id.contains(file_name) {
                println!("Video: Stopping player for closed file: {} (ID: {})", file_name, player_id);
        ...
        if stopped_count > 0 {
            println!("Video: Stopped {} player(s) for closed file: {}", stopped_count, file_name);
        }
    }
}
...
impl VideoPlayer {
    ...
    pub fn new(video_path: &Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let _bus_watch = bus.add_watch(move |_, msg| {
            ...
            match msg.view() {
                MessageView::Error(err) => {
                    println!("Video Error: {} ({})", err.error(), err.debug().unwrap_or_default());
                }
                MessageView::Warning(warn) => {
                    println!("Video Warning: {} ({})", warn.error(), warn.debug().unwrap_or_default());
                }
                MessageView::StateChanged(state) => {
                    if msg.src() == Some(pipeline_debug.upcast_ref()) {
                        println!("Video State changed from {:?} to {:?}", state.old(), state.current());
                    }
                }
                _ => {}
            }
        ...
        println!("Video: Using global audio volume: {:.1}%", initial_volume * 100.0);
