...
impl LspClient {
    ...
    pub fn initialize(&self) -> Result<(), String> {
        ...
        let workspace_uri = workspace_url.as_str().parse::<Uri>()
    ...
    fn send_message(&self, message: &JsonRpcMessage) -> Result<(), String> {
        ...
        if let Some(ref mut child) = *process {
            if let Some(ref mut stdin) = child.stdin {
                ...
                println!("Sent LSP message: {}", message.method.as_ref().unwrap_or(&"response".to_string()));
            }
        }
    ...
    pub fn did_open(&self, uri: Uri, language_id: String, version: i32, text: String) -> Result<(), String> {
    ...
    fn read_messages<R: BufRead>(
        ...
        loop {
            ...
            let content_length: usize = if let Some(len_str) = header_line.strip_prefix("Content-Length: ") {
            ...
            if method == PublishDiagnostics::METHOD {
                if let Some(params) = message.params {
                    if let Ok(diag_params) = serde_json::from_value::<PublishDiagnosticsParams>(params) {
                        println!("Received {} diagnostics for {:?}", diag_params.diagnostics.len(), diag_params.uri.as_str());
