...
use syn::{visit::Visit, ItemFn, Pat, Local, Expr, Ident};
...
impl RustLintVisitor {
    ...
    fn check_naming_convention(&mut self, ident: &Ident, expected_style: &str) {
        ...
        let is_snake_case = name.chars().all(|c| c.is_lowercase() || c.is_numeric() || c == '_');
        let is_upper_case = name.chars().all(|c| c.is_uppercase() || c.is_numeric() || c == '_');
        let is_camel_case = name.chars().next().map_or(false, |c| c.is_uppercase()) 
            && !name.contains('_');
...
pub fn lint_rust_code(code: &str) -> Vec<Diagnostic> {
    ...
    let unused_vars: Vec<String> = visitor.declared_variables
        .iter()
