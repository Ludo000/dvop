...
mod debugger; // Debugger functionality
mod lsp;
...
fn get_menu_commands() -> Vec<MenuCommand> {
    vec![
        MenuCommand {
            label: "New File",
            action: "win.new-file",
            keywords: vec!["new", "file", "create"],
        },
        MenuCommand {
            label: "Open...",
            action: "win.open",
            keywords: vec!["open", "file", "load"],
        },
        MenuCommand {
            label: "Save",
            action: "win.save",
            keywords: vec!["save", "write"],
        },
        MenuCommand {
            label: "Save As...",
            action: "win.save-as",
            keywords: vec!["save", "as", "copy"],
        },
        MenuCommand {
            label: "Close Tab",
            action: "win.close-tab",
            keywords: vec!["close", "tab"],
        },
        MenuCommand {
            label: "Close All Tabs",
            action: "win.close-all-tabs",
            keywords: vec!["close", "all", "tabs"],
        },
        MenuCommand {
            label: "Quit",
            action: "app.quit",
            keywords: vec!["quit", "exit", "close"],
        },
        MenuCommand {
            label: "Find...",
            action: "win.find",
            keywords: vec!["find", "search"],
        },
        MenuCommand {
            label: "Find and Replace...",
            action: "win.find-replace",
            keywords: vec!["find", "replace", "search"],
        },
        MenuCommand {
            label: "Preferences...",
            action: "win.preferences",
            keywords: vec!["preferences", "settings", "options"],
        },
        MenuCommand {
            label: "Explorer",
            action: "win.toggle-explorer",
            keywords: vec!["explorer", "files", "sidebar"],
        },
        MenuCommand {
            label: "Search",
            action: "win.toggle-search",
            keywords: vec!["search", "find", "sidebar"],
        },
        MenuCommand {
        ...
        MenuCommand {
            ...
            action: "win.refresh",
            keywords: vec!["refresh", "reload", "files"],
        },
        MenuCommand {
            label: "New Terminal",
            action: "win.new-terminal",
            keywords: vec!["new", "terminal", "console", "create"],
        },
        MenuCommand {
            label: "Toggle Terminal",
            action: "win.toggle-terminal",
    ...
    app.connect_startup(|_| {
        if let Some(settings) = gtk4::Settings::default() {
            ...
            if settings.is_gtk_application_prefer_dark_theme() != prefer_dark {
                println!(
                    "Warning: GTK dark mode setting didn't match our preference! Trying again..."
                );
                settings.set_gtk_application_prefer_dark_theme(prefer_dark);
...
fn build_ui(app: &Application, file_to_open: Option<PathBuf>) {
    ...
    add_terminal_button.connect_clicked(move |_| {
        ui::terminal::add_terminal_tab_with_toggle(
            &terminal_notebook_for_button,
            None,
            &editor_paned_for_button,
        );
    });
    ...
    drag_gesture.connect_drag_end(move |_, offset_x, _offset_y| {
        ...
        if final_width < 50 {
            ...
            debugger_button_for_drag.set_active(false);
            ...
            if !was_visible || offset_x > 10.0 {
                ...
                if let Some(ref name) = visible_child_name {
                    match name.as_str() {
                        ...
                        "debugger" => {
                            debugger_button_for_drag.set_active(true);
                            explorer_button_for_drag.set_active(false);
                            search_button_for_drag.set_active(false);
                            git_diff_button_for_drag.set_active(false);
                        },
    ...
    let debugger_button_for_monitor = debugger_button.clone();
    paned.connect_position_notify(move |p| {
        ...
        if let Some(start_child) = p.start_child() {
            if start_child.is_visible() && position < 50 && position > 0 {
                ...
                debugger_button_for_monitor.set_active(false);
            }
        }
    });
    ...
    if let Some(debugger_panel_widget) = sidebar_stack.child_by_name("debugger") {
        if let Some(debugger_panel_box) = debugger_panel_widget.downcast_ref::<gtk4::Box>() {
            ...
            let debugger_panel = crate::debugger::ui::create_debugger_panel(
                current_dir.borrow().clone(),
                terminal_notebook_template.clone(),
            );
    ...
    initial_text_buffer.connect_changed(move |_buffer| {
        ...
        else if label_text.starts_with('*')
            ...
            handlers::get_active_text_view_and_buffer(&editor_notebook_for_find)
        {
            if let Ok(source_buffer) = text_buffer.downcast::<sourceview5::Buffer>() {
                if let Ok(source_view) = text_view.downcast::<sourceview5::View>() {
                    crate::search::show_find_only_for_buffer(
                        Some(&source_buffer),
                        Some(&source_view),
                    );
                } else {
    ...
    new_terminal_action.connect_activate(move |_, _| {
        ...
        ui::terminal::add_terminal_tab_with_toggle(
            ...
            None,
            &editor_paned_clone_for_new_terminal,
        );
    });
    ...
    save_main_button.connect_clicked(move |_| {
            ...
            handlers::get_active_text_view_and_buffer(&editor_notebook_clone)
        {
            ...
            let path_to_save_opt = file_path_manager_clone
                .borrow()
                .get(&current_page_num)
                .cloned();
        ...
        *active_tab_path_clone_for_switch.borrow_mut() = new_active_path.clone();
        ...
        if let Some(file_path) = &new_active_path {
            ...
            if let Some((text_view, _)) =
                handlers::get_text_view_and_buffer_for_page(notebook, page_num)
            {
                if let Some(source_view) = text_view.downcast_ref::<sourceview5::View>() {
                    update_cursor_position_status(
                        source_view,
                        &secondary_status_label_clone,
                        &filename,
                        Some(file_path),
                    );
                ...
                if *current_dir_clone_for_switch.borrow() != parent_path {
                    *current_dir_clone_for_switch.borrow_mut() = parent_path.clone();
                    ...
                    utils::update_file_list(
                        ...
                        &current_dir_clone_for_switch.borrow(),
                        &new_active_path,
                        utils::FileSelectionSource::TabSwitch,
                    );
                    ...
                    utils::update_path_buttons(
                        &path_box_clone_for_switch,
                        &current_dir_clone_for_switch,
                        &file_list_box_clone_for_switch,
        ...
        // Rebind global search context to the new tab's buffer if search UI is currently visible
        {
            ...
            if search_state.search_bar.is_search_mode() {
                if let Some((text_view, text_buffer)) =
                    handlers::get_text_view_and_buffer_for_page(notebook, page_num)
                {
                    if let (Ok(source_view), Ok(source_buffer)) = (
                        text_view.downcast::<sourceview5::View>(),
                        text_buffer.downcast::<sourceview5::Buffer>(),
                    ) {
        ...
        if let Some((_, _)) = handlers::get_text_view_and_buffer_for_page(notebook, page_num) {
            ...
            utils::update_save_menu_button_visibility(&save_menu_button_clone_for_switch, None);
        }

        // Update volume control visibility based on the new active tab
        ui::update_volume_control_visibility_for_tab(
            &volume_control_clone_for_switch,
            &new_active_path,
        );
    });
    ...
    terminal_button.connect_clicked(move |_| {
        ...
        ui::terminal::add_terminal_tab_with_toggle(
            &terminal_notebook_clone_for_terminal_button,
            Some(current_dir_clone_for_terminal_button.borrow().clone()),
            &editor_paned_clone_for_terminal_button2,
        );
    });
    ...
    add_file_button.connect_clicked(move |_| {
        ...
        let deps_for_new_tab_creation = handlers::NewTabDependencies {
            ...
            window: window_clone_for_add_file
                .clone()
                .upcast::<ApplicationWindow>(),
...
fn setup_gsettings_monitor(window: &impl IsA<gtk4::Widget>, terminal_notebook: &gtk4::Notebook) {
    ...
    match std::panic::catch_unwind(|| gio::Settings::new("org.gnome.desktop.interface")) {
                    ...
                    settings.connect_changed(Some("color-scheme"), move |_, _| {
                        ...
                        glib::timeout_add_local_once(
                            ...
                            move || {
                                ...
                                ui::terminal::update_all_terminal_themes(
                                    &terminal_notebook_clone_inner,
                                );
                            },
                        );
                    });
        ...
        Err(_) => {
            println!(
                "Could not set up GSettings monitor - org.gnome.desktop.interface not available"
            );
        }
    }
}
